...
{ // In this function we
    // have to pass matrix as reference otherwise the memory would not be
    // allocated dynamically to the matrix. The changes we made to the matrix
    // would not be take affect if we pass matrix by value; thus matrix
    // must be passed by reference, otherwise when deleMatrix function is
    // called it would not be able to deallocate memory because memory
    // has not been allocated at first place; which wil end up in an
    // undefined behaviour
    int init = 10;
    matrix = new int *[m];
    for (int i = 0; i < m; i++)
